...
export default function AddSupplierModal({open,onClose}:{open:boolean;onClose:()=>void}){
  ...
  return(
    <Modal open={open} title="New Supplier" onClose={onClose} onSubmit={save}
           submitLabel="Add supplier" width={420}>
      <label>Image
        <input type="file" accept="image/*" onChange={pickImg}/>
      </label>

      <label>Supplier name*
        <input placeholder="Enter supplier name" value={name}
               onChange={e=>setName(e.target.value)}/>
        {submitted&&errors.name&&<small className="err">{errors.name}</small>}
      </label>

      <label>Contact number*
        <input placeholder="Phone #" value={contact}
               onChange={e=>setContact(e.target.value.replace(/\D/g,''))}/>
        {submitted&&errors.contact&&<small className="err">{errors.contact}</small>}
      </label>

      <label>Email*
        <input placeholder="someone@mail.com" value={email}
               onChange={e=>setEmail(e.target.value)}/>
        {submitted&&errors.email&&<small className="err">{errors.email}</small>}
      </label>
    </Modal>
  );
}
