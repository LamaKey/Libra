...
export default function AddProductModal({open,onClose}:{open:boolean;onClose:()=>void;}){
  ...
  const save=()=>{
    ...
    const rec:Product={
      ...
      name,
      price:+price,
      qty: 0,                    // ← will be set by simulation
      unitWeight: 0,             // ← set by calibration wizard
      low: false,
      suppliers,
      location,
      img,
  ...
  return(
    <Modal open={open} title="New Product" onClose={onClose}
           width={520} onSubmit={save} submitLabel="Add product">
      ...
      <label>Buying price*
        <input value={price} onChange={e=>setPrice(numberOnly(e.target.value))}
               placeholder="0.00"/>
        {submitted&&errors.price&&<small className="err">{errors.price}</small>}
      </label>

      <label>Location
        <SearchSelect value={location} onChange={setLoc}
                      options={branchOptions()} placeholder="Select branch"/>
      </label>

      <label>Suppliers*
        <SearchSelect value={suppliers} onChange={setSup}
                      options={supplierOpts} multi placeholder="Select suppliers"/>
        {submitted&&errors.suppliers&&<small className="err">{errors.suppliers}</small>}
      </label>

      <label>Currency
        <SearchSelect value={currency} onChange={setCur}
                      options={[{value:'USD',label:'USD'},{value:'EUR',label:'EUR'},{value:'GBP',label:'GBP'}]}/>
      </label>

      <p style={{fontSize:13,color:'#666',marginTop:24}}>
        Stock quantity is calculated automatically from scale readings.</p>
    </Modal>
  );
}
