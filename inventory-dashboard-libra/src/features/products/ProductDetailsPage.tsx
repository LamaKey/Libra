...
export default function ProductDetailsPage() {
  ...
  /* ---------- suppliers ---------- */
  const suppliers = query<{ id: string; name: string; email: string; contact: string }>('suppliers');

  /* ---------- dynamic stock locations ---------- */
  const scales    = query<Scale>('scales').filter(s => s.productId === prod.id);
  ...
  const stockRows = (() => {
    ...
    const map = new Map<string, number>();          // branch-id → stock
    scales.forEach(sc => {
      const room = rooms.find(r => r.id === sc.roomId);
      if (!room) return;
      map.set(room.location, (map.get(room.location) || 0) + prod.qty / scales.length);
    });
    return [...map.entries()].map(([bid, stock]) => {
      const br = branches.find(b => b.id === bid);
      return { name: br?.name ?? '—', stock: Math.round(stock) };
    });
  })();
    ...
    <div className={styles.page}>
          ...
          <section className={styles.card}>
            ...
            <table className={styles.tableMini}>
              <tbody>
                ...
                <tr><th>Product ID</th>      <td>{prod.id}</td></tr>
                <tr><th>Currency</th>        <td>{prod.currency ?? D.DEFAULT_CURRENCY}</td></tr>
                <tr><th>Unit weight</th>     <td>{prod.unitWeight} g</td></tr>
                <tr><th>Crate weight</th>    <td>{crateWeight} g</td></tr>
                <tr><th>Units / crate</th>   <td>{unitsPerCrate}</td></tr>
                <tr><th>Threshold value</th> <td>{threshold}</td></tr>
              </tbody>
            </table>
          </section>
      ...
      <div className={styles.grid2}>
        <section className={styles.card}>
          ...
          <table className={styles.tableMini}>
            <tbody>
              <tr><th>Opening stock</th>   <td>{opening}</td></tr>
              <tr><th>Remaining stock</th> <td>{liveQty}</td></tr>
              <tr><th>On the way</th>      <td>{prod.onTheWay ?? 0}</td></tr>
              <tr><th>Threshold value</th> <td>{threshold}</td></tr>
            </tbody>
          </table>
        </section>
      ...
      {/* ======= locations & alerts side-by-side ======= */}
      <div className={styles.grid2}>
        {/* stock locations */}
        <section className={styles.card}>
          <h3>Stock locations <a href="/rooms">See all</a></h3>
          <table className={styles.tableMini}>
            <thead>
              <tr><th>Branch</th><th style={{ textAlign: 'right' }}>Stock in hand</th></tr>
            </thead>
            <tbody>
              {stockRows.map(r => (
                <tr key={r.name}>
        ...
        {/* alerts */}
        <section className={styles.card}>
          <h3>Alerts <a href="/alerts">See all</a></h3>
          <table className={styles.tableMini}>
            <thead><tr><th>Alert</th><th>Branch</th></tr></thead>
            <tbody>
              {alerts.map(a => {
      ...
      {/* edit-modal */}
      <EditProductModal product={prod} open={edit} onClose={() => setEdit(false)} />
    </div>
  );
}
